...
public interface TaskMapper extends BaseMapper<Task>{
    ...
    @Update({"update task set status = #{newStatus}, retry_times = #{retryTimes} where id = #{id}"})
    Integer updateStatus(@Param("retryTimes") int retryTimes, @Param("id") Long id, @Param("newStatus") Integer newStatus, @Param("oldStatus") Integer oldStatus);
